...
export class VColor {
    ...
    ToHex_RGB() {
        ...
        if (result.length > 6)
            result = result.slice(0, -2); // remove alpha part (if present)
        return result;
    }
    ToHex_RGBA() {
        let result = this.ToChromaJS().hex().slice(1); // remove "#" prefix (redundant)
...
export function GetPixelColors(positions) {
