...
export function GetPixelColors_Fast(positions: Vector2[]) {
...
export function CaptureScreenshotOfRectContainingAllPositions(positions: Vector2[]) {
...
export class Screenshot {
	...
	GetPixel(...args) {
		const pos = args.length == 1 ? args[0] as Vector2 : new Vector2(args[0], args[1]);
		const pixelsInPreviousRows = this.width * pos.y;
		const pixelIndex = pixelsInPreviousRows + pos.x;
		return new VColor(
			// yeah, the order is a bit odd
			this.buffer[(pixelIndex * 4) + 2],
			this.buffer[(pixelIndex * 4) + 1],
			this.buffer[(pixelIndex * 4) + 0],
			this.buffer[(pixelIndex * 4) + 3],
		);
	}
}
...
export function CaptureScreenshot(opt: {
	...
	try {
		if (opt.windowHandle == null) opt.windowHandle = user32.GetDesktopWindow();
		...
		opt.rectToCapture = opt.rectToCapture ?? new VRect(0, 0, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top);
		...
		if (hdcWindow != null) user32.ReleaseDC(opt.windowHandle, hdcWindow);
